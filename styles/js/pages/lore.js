  ...
  let dex = await charadex.initialize.page(
    ...
    (arr) => {
      ...
      for (let lore of arr) {
        ...
        lore["mvpprofiles"] = `<a class="btn btn-primary btn-sm" href="${charadex.url.addUrlParameters(
          charadex.url.getPageUrl(charadex.page.masterlist.sitePage),
          { profile: charadex.tools.scrub(lore.mvp) })}">${lore.mvp}</a>`;

        lore["probationprofiles"] = `<a class="btn btn-info btn-sm" href="${charadex.url.addUrlParameters(
          charadex.url.getPageUrl(charadex.page.masterlist.sitePage),
        ...
        for (let participant of lore.participation) {
          ...
          participationArr.push(`<a class="btn btn-warning btn-sm" href="${url}">${participant}</a>`);
        }
